...
- [x] **Phase 10: Discipline + citation-style breadth + Zotero MCP** - Full CSL catalog, RIS export, remaining disciplines, Zotero MCP source provider (completed 2026-06-22)
...
- [x] 03-05-PLAN.md (wave 4) -- 8 hash-pinned prompts in templates/prompts/ + apa.csl + disciplines.json preset + 9 new hash-pins (D-12, D-13, D-19, INTK-04)
...
- Wave 2: 04-03 (write orchestration; depends 04-01)
- Wave 3: 04-04 (revise-swap + --research; depends 04-01, 04-02, 04-03 — serialized after 04-03 to avoid shared tests/tier-contract.test.ts edit)
- Wave 4: 04-05 (compile pipeline + tier-contract parity; depends 04-01, 04-02, 04-03, 04-04)
...
- [x] 04-05-PLAN.md (wave 4) — Compile pipeline: refuse-gate (COMP-01) + outline-order concat (COMP-02) + N-1 token-safe smoothing (COMP-03) + consistency flags (COMP-04) + citation density vs discipline target (COMP-05) + bib regen + DRAFT.md/COMPILE-REPORT.md emission (COMP-07) + tier-contract parity (ARCH-20) (depends 04-01, 04-02, 04-03, 04-04)
...
- [x] 07-03-PLAN.md (wave 1) — hooks upgrade: SessionStart resume emitter + Stop lock-release/log-flush + PreCompact 10s Promise.race + closeSessionLog() export; HOOK-03 coverage-check only (HOOK-01..04)
- [x] 07-04-PLAN.md (wave 2) — NL-routing skills (skills/pensmith.md + plumbing namespace) + plugin.json skills array + tier-contract verb-shortcut/plumbing parity; human-verify live NL routing (UX-02, UX-03, UX-04, UX-05)
...
- [x] 08-03-PLAN.md (wave 1) — pymupdf shellout (graceful-absent) + extractPdfText image-only fallback + pin guard (RSCH-05)
...
- [x] 10-01-PLAN.md (wave 1) — citations.ts renderStyle + resolveStyleName + Map memoization for all 8 styles, deterministic + offline (CITE-02, CITE-03)
- [x] 10-02-PLAN.md (wave 1) — ris-write.ts writeRis (D-19/D-07) + exporter risCopied + research-time CITATIONS.ris emission (CITE-05)
- [x] 10-03-PLAN.md (wave 1) — zotero-mcp source adapter (absence-non-breaking) + tri-state doctor probe (no key leak) + disciplines.json 6-field completion + CS apa→ieee (RSCH-06, CITE-02)
- [x] 10-04-PLAN.md (wave 2) — intake-clarifier 8-style prompt + atomic WN-3 hash re-pin + research.md Zotero fallback + tier-contract RIS parity + 16-verb bijection (CITE-02, CITE-03, RSCH-06)
...
| 10. Discipline + citation-style breadth + Zotero MCP | 5/5 | Complete   | 2026-06-22 |
