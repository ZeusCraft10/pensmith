...
- [x] **Phase 16: CI/DX parity + docs & packaging** — prebuild-first `npm run check`, fresh-clone CI gate, coverage gate, real README + disclaimer, stub workflow bodies filled, packaging cleanup (completed 2026-06-24)
...
- [x] 15-02-PLAN.md — Wave 2: HARD-02 SSRF guard (checkSsrf DNS-preflight + redirect re-check) + HARD-06 FIFO TokenBucket — both in http.ts (one plan, no parallel same-file edits)
- [x] 15-03-PLAN.md — Wave 2: HARD-01 lock.ts stubFor canonicalization (resolve + realpath + win32 case-fold) + compile.ts 'compile:' de-prefix
- [x] 15-04-PLAN.md — Wave 2: HARD-03 pii.ts deepRedactPii + session-log.ts buildRecord recursive string-leaf redaction before write
- [x] 15-05-PLAN.md — Wave 2: HARD-04b pdf-text.ts byte cap + Promise.race wall-clock timeout around the pdf-parse call
- [x] 15-06-PLAN.md — Wave 2: HARD-04c Pass-2/Pass-4 prompt-injection fencing + WN-3 lockstep re-pin (repo-files.test.ts + prompt-loader.ts)
- [x] 15-07-PLAN.md — Wave 3: HARD-05 GPTZero disclosure + consent gate + size cap (honesty.ts) + honesty-framing.md disclosure + standalone re-pin
...
- [x] 16-04-PLAN.md — Wave 4: DOCS-02 — fill doctor/status/next/resume bodies + refresh http-crossref-ping/doctor-output.md (WN-3 re-pin) + PRIVACY.md; bijection + tier-contract intact
...
| 16. CI/DX parity + docs & packaging | v0.2.0 | 4/4 | Complete   | 2026-06-24 |
