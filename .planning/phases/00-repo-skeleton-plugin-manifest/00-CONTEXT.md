...
- **D-17:** `.claude-plugin/marketplace.json` ships the same metadata in marketplace shape; both files validated by a tiny Node script in `scripts/validate-plugin-manifest.js` that runs in CI (D-11). The script reads the upstream Claude Code plugin schema once at lock time and asserts JSON-schema conformance.
- **D-18:** `.mcp.json` declares the pensmith MCP server with `command: "node"`, `args: ["dist/mcp/server.js"]`. The actual `mcp/server.ts` Phase 0 file exists only as a stub that exports an empty server — full read-only resources land in Phase 2 per ROADMAP.md.
