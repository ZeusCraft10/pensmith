...
- **D-05:** **Full verb dispatcher with `doctor`-only real handler.** Both Tier 1 slash-command and Tier 2 citty dispatcher register **all 16 verbs from UX-02** (`new` / `next` / `status` / `research` / `outline` / `plan` / `write` / `verify` / `compile` / `done` / `resume` / `list` / `open` / `sketch` / `add` / `doctor`); 15 of them return `NotYetImplemented(verb, phase: N)`. Phase 5 wires bare `/pensmith` state-aware routing — explicit conscious rejection here, not accidental omission. *(2026-05-16 correction: the original draft of this decision said "17 verbs" but enumerated 16 names — REQUIREMENTS.md UX-02 is the source of truth and pins exactly 16. Phase 6+ verbs like `export` / `citations` / `humanize` / `gpt-zero` / `plagiarism` are sub-commands under `compile`/`verify`, NOT first-class verbs in v0.1.0.)*
...
- **D-13:** **Doctor is deterministic code; workflow body is a thin handler shell.** Doctor entry exports `runDoctor(): Promise<Record<string, ProbeResult>>` (Record-keyed-by-`probe.id` per D-20 — see naming note below). `workflows/doctor.md` is a thin pointer ("invoke the doctor probes; render report"). Tier 1: slash command → `paper_capability_probe` MCP tool → `runDoctor()`. Tier 2: citty verb → `runDoctor()` direct. No LLM in doctor's hot path; DOCT-06 tier-equivalence becomes structural rather than statistical. Establishes "deterministic workflow body" as a first-class shape alongside "LLM-driven workflow body" used in later phases. *(2026-05-16 naming reconciliation: the original draft said `bin/lib/doctor.ts` + `runAllProbes()` + `ProbeReport`. Planner laid out the implementation as `bin/lib/doctor/probes.ts` orchestrator + per-probe sub-modules in `bin/lib/doctor/probes/*.ts` + `runDoctor()` returning `Record<string, ProbeResult>` to satisfy D-20's Record-keyed-by-id requirement. Final canonical names: `bin/lib/doctor/probes.ts::runDoctor()` returning `Record<string, ProbeResult>`. 02-04's `paper_capability_probe` tool imports from `bin/lib/doctor/probes.ts`.)*
